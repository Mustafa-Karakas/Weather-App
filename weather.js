...
const getWeatherDataFromApi = async () => {
    ...
    // input.value = ""; //alttakinin aynısı. form kullandığımız için 1 den fazla input olması halinde form.reset kullanırsak hepsinin içini boşaltır. sayfa yeniendiğinde boş olur
    ...
    try {
        ...
        const response = await axios(url); // üstteki ile aynı şeydir. üstteki fetch ile yazım bu ise axios ile yazımdır. Axios daha sadedir. reactta axios kullanılır. Javascriptte ise fetch kullanılır.
        const { name, main, sys, weather } = response.data;
        // console.log(response.data);
        let iconUrl = `http://openweathermap.org/img/wn/${weather[0].icon}@2x.png`;

        //forEach => array + nodeList te kullanılabilir
        //map, filter, reduce => array de kullanılabilir
